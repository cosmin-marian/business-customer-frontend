  ...
  def testReviewDetailsController = {
    val mockDataCacheConnector = new DataCacheConnector {
      ...
      override def fetchAndGetBusinessDetailsForSession(implicit hc: HeaderCarrier) = {
        reads = reads + 1
        Future.successful(Some(ReviewDetails("ACME", Some("Limited"), address, "sap123", "safe123", Some("agent123"))))
      }
    }
  ...
  "continue " must {
    ...
    "Authorised Users" must {
      ...
      "return service start page correctly for AMLS Users" in {
        ...
        continueWithAuthorisedUser("AMLS") {
          result =>
            status(result) must be(SEE_OTHER)
            redirectLocation(result).get must include("/anti-money-laundering")
        }
      }      
