  ...
  "ErrorMessageUtils" must {
    "return true" when {
      "error number is 9001 and message is duplicate agent error" in {
        ...
        parseErrorResp(HttpResponse(502, Some(badGatewayResponse)), uniqueAgentErrorNum, uniqueAgentErrorMsg) must be (true)
      }
    }
    ...
    "return false" when {
      "error number is 9002 and message is duplicate agent error" in {
        ...
        parseErrorResp(HttpResponse(502, Some(badGatewayResponse)), "9002", uniqueAgentErrorMsg) must be (false)
      }
    }
  }
